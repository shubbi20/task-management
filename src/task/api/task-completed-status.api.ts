...
const completedTaskHandler = async (req: any, res: any, next: any) => {
    ...
    try {
        const pendingTasks = await taskService.updateTaskToCompleted(task);
...
router.patch('/tasks/:taskId/completed',authenticate,authorize(USER_ROLE_TYPE.ADMIN,USER_ROLE_TYPE.SIMPLE), completedTaskHandler);
