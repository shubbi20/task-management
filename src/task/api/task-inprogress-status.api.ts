...
const inProgressTaskHandler = async (req: any, res: any, next: any) => {
    ...
    try {
        const pendingTasks = await taskService.updateTaskToInProgress(task);
...
router.patch('/tasks/:taskId/inProgress',authenticate,authorize(USER_ROLE_TYPE.ADMIN,USER_ROLE_TYPE.SIMPLE), inProgressTaskHandler);
